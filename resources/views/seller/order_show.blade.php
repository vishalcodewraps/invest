      ...
      <div>
        <div class="row row-gap-4">
                ...
                @if ($order->approved_by_seller == 'pending')
                    ...
                    @if ($order->order_status != 'cancel_by_buyer' )
                        <div class="d-flex gap-3 align-items-center">
                          <button type="button" class="w-btn-secondary-lg" id="approvedOrderBtn">
                            {{ __('translate.Approved Now') }}
                        </button>

                        <button type="button" class="w-btn-secondary-lg rejected_btn" id="rejectedOrderBtn">
                            {{ __('translate.Rejected Order') }}
                        </button>
                        </div>

                        <form action="{{ route('seller.order-approved', $order->id) }}" class="hidden" method="POST" id="approvedForm">
                                @csrf
                        </form>

                        <form action="{{ route('seller.order-rejected', $order->id) }}" class="hidden" method="POST" id="rejectedForm">
                                @csrf
                        </form>
                    @endif


                @elseif($order->approved_by_seller == 'approved')
                    ...
                    @if ($order->order_status == 'approved_by_seller')
                        ...
                        <form action="{{ route('seller.order-cancel', $order->id) }}" class="hidden" method="POST" id="cancelForm">
                                @csrf
                        </form>
                    @endif
          ...
          <div class="col-lg-4 col-md-6">
            ...
            @if ($order->approved_by_seller == 'approved')
            <div class="info-card-wrapper mt-3 order-file-box">
              <div class="info-card-header">
                <h4 class="info-card-title">{{ __('translate.File Submission') }}</h4>
              </div>
              <div class="info-card-body">
                <form action="{{ route('seller.order-submission', $order->id) }}" method="POST" enctype="multipart/form-data">
                  ...
                  @if ($order->submit_file)
                    <a
                      href="{{ route('download-submission-file', $order->submit_file) }}"
          ...
          <div class="col-lg-4 col-md-6">
            <div class="info-card-wrapper">
              <div class="info-card-header">
                <h4 class="info-card-title">{{ __('translate.Buyer Info') }}</h4>
              </div>
              <div class="info-card-body">
                <div class="d-flex align-items-center gap-4 pb-3">
                    @if ($buyer->image)
                        <img src="{{ asset($buyer->image) }}" class="seller-avatar" alt="" />
                    @else
                  ...
                  <div>
                    <h3 class="card-seller-name">
                      {{ html_decode($buyer->name) }} <span>({{ html_decode($buyer->designation) }})</span>
                    </h3>
                    <p class="seller-location">{{ html_decode($buyer->address) }}</p>
                  </div>
                </div>
                <ul class="seller-info-list">
                  <li class="d-flex align-items-center seller-info-list-item justify-content-between">
                    ...
                    <div>
                      <p>{{ $buyer->created_at->format('d M Y') }}</p>
                    </div>
                  </li>
                  <li
...
<script>
    (function($) {
        ...
        $(document).ready(function () {
            ...
            $('#approvedOrderBtn').on('click', function(){
                Swal.fire({
                    title: "{{ __('translate.Are you really want to approved the order?') }}",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: "{{ __('translate.Yes, Approved It') }}",
            ...
            $('#rejectedOrderBtn').on('click', function(){
                Swal.fire({
                    title: "{{ __('translate.Are you really want to rejected the order?') }}",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: "{{ __('translate.Yes, Rejected It') }}",
                    cancelButtonText: "{{ __('translate.Cancel') }}",
                }).then((result) => {
                    if (result.isConfirmed) {
                        $("#rejectedForm").submit();
